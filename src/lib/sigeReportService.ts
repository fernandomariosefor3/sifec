...
//      duplicada.
//
// Correções da integração ao PR #19 (fusão com a reestruturação SIFEC):
//   A) matrícula atual para turma NOVA passa a ser OBRIGATÓRIA — a versão
//      anterior só validava o valor QUANDO informado, deixando passar
//      `undefined` (campo vazio) e silenciosamente gravando matriculaInicial
//      0. Agora vazio é rejeitado explicitamente; zero explícito continua
//      aceito; só inteiro não negativo;
//   B) fase 2 (grade_entry_monitoring) deixa de usar um ÚNICO WriteBatch
//      para todas as turmas do relatório — processada em chunks de no
//      máximo SIGE_REPORT_CHUNK_SIZE linhas, cada chunk num batch próprio e
//      atômico (documento + audit_log de cada linha, juntos). Um relatório
//      grande (dezenas de turmas) nunca arrisca exceder o custo de
//      avaliação de regra por escrita nem o limite de operações por batch.
//      SigeReportPartialSaveError agora também informa quais linhas já
//      foram salvas, quais ainda estão pendentes, e qual chunk falhou.
...
// Correção da integração ao PR #19, item B: máximo de linhas por chunk da
// fase 2 — cada chunk grava documento + audit_log de cada linha no mesmo
// WriteBatch (atômico dentro do chunk), mas chunks diferentes são commits
// SEPARADOS e sequenciais (nunca atomicidade entre chunks — mesma limitação
// já documentada para as duas fases do fluxo inteiro).
export const SIGE_REPORT_CHUNK_SIZE = 8;

// Lançado quando pelo menos um chunk da fase 2 já foi commitado (ou a fase
// 1 criou turmas), mas um chunk posterior falhou — nunca um erro genérico
// aqui, porque o chamador PRECISA saber exatamente o que já existe de fato
// para não tentar recriar/regravar numa nova tentativa (ver SigeReportModal,
// que usa createdTurmas/savedRows para bloquear um novo clique até a lista
// de turmas e acompanhamentos ser atualizada).
export class SigeReportPartialSaveError extends Error {
  readonly createdTurmas: readonly CreatedTurmaInfo[];
  readonly savedRows: readonly SigeReportRowResult[];
  readonly pendingRows: readonly SigeReportRowInput[];
  readonly failedChunkIndex: number;
  readonly originalError: unknown;

  constructor(
    createdTurmas: readonly CreatedTurmaInfo[],
    savedRows: readonly SigeReportRowResult[],
    pendingRows: readonly SigeReportRowInput[],
    failedChunkIndex: number,
    originalError: unknown
  ) {
    super(
      `As turmas novas foram criadas e ${savedRows.length} turma(s) já tiveram o acompanhamento gravado, mas o ` +
      `lote ${failedChunkIndex + 1} falhou: ` +
      (originalError instanceof Error ? originalError.message : String(originalError)) +
      ` ${pendingRows.length} linha(s) ainda não foram gravadas. A lista de turmas e acompanhamentos será ` +
      'atualizada automaticamente — nada já gravado será duplicado numa nova tentativa.'
    );
...
export interface SigeReportRowInput {
  ...
  turmaNome: string;
  turno?: string;
  // Correção da integração ao PR #19, item A: OBRIGATÓRIO quando a linha
  // cria uma turma NOVA (vira matriculaInicial/matriculaAtual do novo
  // documento) — vazio é rejeitado, zero explícito é aceito. Para turma
  // existente é ignorado pelo serviço — a interface mostra o campo como
  // somente informativo nesse caso.
...
// e só depois os documentos de grade_entry_monitoring (fase 2, processada
// em chunks — ver SIGE_REPORT_CHUNK_SIZE). Cada chunk é atômico
// isoladamente, mas NÃO há atomicidade completa entre fase 1 e fase 2, nem
// entre chunks diferentes da fase 2 (limitação real do modelo de regras do
// Firestore, não uma escolha de implementação). Qualquer falha depois de
// algo já ter sido commitado lança SigeReportPartialSaveError (nunca um
// erro genérico) com o que já existe de fato, para o chamador poder se
// recuperar sem duplicar nada numa nova tentativa.
export async function saveSigeReport(
  ...
  // Correção da integração ao PR #19, item A: matrícula atual passa a ser
  // OBRIGATÓRIA para toda linha de turma NOVA — vazio é rejeitado
  // explicitamente (a versão anterior só validava o valor quando presente,
  // deixando `undefined` passar e gravar matriculaInicial 0 em silêncio).
  // Zero explícito continua aceito; só inteiro não negativo. Validado ANTES
  // de qualquer escrita.
  for (const row of resolved) {
    if (row.isNewTurma) {
      if (row.input.matriculaAtual === undefined || !isNonNegativeInteger(row.input.matriculaAtual)) {
        throw new SigeReportValidationError(
          `Informe a matrícula atual da turma nova "${row.input.turmaNome}" — o campo não pode ficar vazio (zero é aceito), e precisa ser um número inteiro maior ou igual a zero.`
        );
      }
    }
  }
  ...
  // Correção da integração ao PR #19, item B: chunks de no máximo
  // SIGE_REPORT_CHUNK_SIZE linhas, cada um num WriteBatch próprio —
  // NUNCA um único batch para todas as turmas do relatório.
  const results: SigeReportRowResult[] = [];
  for (let chunkStart = 0; chunkStart < prepared.length; chunkStart += SIGE_REPORT_CHUNK_SIZE) {
    const chunk = prepared.slice(chunkStart, chunkStart + SIGE_REPORT_CHUNK_SIZE);
    const chunkBatch = writeBatch(db);
    const chunkResults: SigeReportRowResult[] = [];
    for (const { index, row, existingMonitoring, saveInput } of chunk) {
      ...
      chunkBatch.set(doc(db, 'grade_entry_monitoring', payload.id), payload);
    ...
    try {
      ...
      const failedChunkIndex = chunkStart / SIGE_REPORT_CHUNK_SIZE;
      const pendingRows = prepared.slice(chunkStart).map(p => p.row.input);
      throw new SigeReportPartialSaveError(createdTurmas, results, pendingRows, failedChunkIndex, err);
    }
