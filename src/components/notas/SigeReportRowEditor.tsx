...
export function computeRow(row: SigeReportRowDraft, existingTurmas: readonly Turma[]): RowComputed {
  ...
  const isFullyResolved = row.turmaNome.trim() !== '' && !resolution.needsUserChoice &&
    (resolution.resolvedTurmaId != null || resolution.isNovaTurmaConfirmada);

  return { match, resolution, allFieldsFilled, sumMatches, entriesConsistent, studentsSum, percentage, resultingStatus, isMathematicallyValid, isFullyResolved };
}
