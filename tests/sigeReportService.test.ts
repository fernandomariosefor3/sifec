...
//
// Integração ao PR #19: matrícula atual passa a ser obrigatória para turma
// nova (vazio rejeitado, zero explícito aceito) e a fase 2 é processada em
// chunks de no máximo SIGE_REPORT_CHUNK_SIZE linhas — cada chunk é um
// WriteBatch próprio, nunca um único batch para todas as turmas do
// relatório. SigeReportPartialSaveError agora também informa savedRows/
// pendingRows/failedChunkIndex.
...
describe('sigeReportService', () => {
  ...
  describe('saveSigeReport', () => {
    ...
    it('turma existente não gera escrita na coleção turmas nem sua auditoria — só o acompanhamento', async () => {
      ...
      // Só UM writeBatch (fase 1 não roda quando não há turma nova; fase 2
      // com 1 linha cabe num único chunk).
    ...
    it('múltiplas turmas no mesmo relatório: mistura de turma existente e turma nova é tratada corretamente', async () => {
      ...
      expect(monitoringBatch.set).toHaveBeenCalledTimes(2); // as duas linhas, mesmo chunk
      expect(result.turmasCreated).toBe(1);
    ...
    it('nunca escreve na coleção grade_entry_monitoring_disciplina (compatibilidade com a dimensão por disciplina do PR #19)', async () => {
      const { saveSigeReport } = await import('../src/lib/sigeReportService');
      setFreshTurmas([turma({ id: 't1' })]);
      const input = baseInput({ rows: [rowExisting()] });
      await saveSigeReport(input, new Map());

      const collectionsWritten = mockDoc.mock.calls
        .filter(args => args.length === 3)
        .map(args => args[1]);
      expect(collectionsWritten).not.toContain('grade_entry_monitoring_disciplina');
      expect(collectionsWritten.every((c: string) => c === 'grade_entry_monitoring')).toBe(true);
    });

    it('payload gravado nunca contém nome de estudante, matrícula individual ou nota individual — só totais agregados', async () => {
      const { saveSigeReport } = await import('../src/lib/sigeReportService');
      setFreshTurmas([turma({ id: 't1' })]);
      const input = baseInput({ rows: [rowExisting()] });
      await saveSigeReport(input, new Map());
      ...
      const [monitoringBatch] = batchInstances;
      const [, payload] = monitoringBatch.set.mock.calls[0];
  ...
  // Item 6 do code review do PR #18 + integração ao PR #19 (item A: campo
  // vazio agora é REJEITADO para turma nova, nunca mais default para zero).
  describe('matrícula atual', () => {
    ...
    it('matrícula atual inválida (texto/NaN) em turma nova bloqueia antes de qualquer escrita', async () => {
      ...
      await expect(saveSigeReport(input, new Map())).rejects.toThrow(/Informe a matrícula atual/);
      expect(mockWriteBatch).not.toHaveBeenCalled();
    });
    ...
    // Correção da integração ao PR #19, item A: campo VAZIO (undefined)
    // para turma nova agora é REJEITADO explicitamente — nunca mais
    // silenciosamente vira matriculaInicial 0.
    it('campo vazio (undefined) em turma nova é rejeitado — nunca mais usa zero por default', async () => {
      const { saveSigeReport } = await import('../src/lib/sigeReportService');
      setFreshTurmas([]);
    ...
    it('zero EXPLÍCITO em turma nova continua aceito (distinto de vazio)', async () => {
      const { saveSigeReport } = await import('../src/lib/sigeReportService');
      setFreshTurmas([]);
      const input = baseInput({ rows: [rowNova({ matriculaAtual: 0 })] });
  ...
  // Correção da integração ao PR #19, item B: fase 2 processada em chunks
  // de no máximo SIGE_REPORT_CHUNK_SIZE (8) linhas — nunca um único batch
  // para todas as turmas do relatório.
  describe('processamento em chunks (relatórios grandes)', () => {
    it('relatório com 25 turmas existentes é dividido em 4 chunks de no máximo 8 linhas (8+8+8+1)', async () => {
      const { saveSigeReport, SIGE_REPORT_CHUNK_SIZE } = await import('../src/lib/sigeReportService');
      expect(SIGE_REPORT_CHUNK_SIZE).toBe(8);
      const turmas = Array.from({ length: 25 }, (_, i) => turma({ id: `t${i + 1}`, nome: `Turma ${i + 1}` }));
      setFreshTurmas(turmas);
      const rows = turmas.map(t => rowExisting({ turmaId: t.id, turmaNome: t.nome }));
      const input = baseInput({ rows });

      const result = await saveSigeReport(input, new Map());

      expect(result.rows).toHaveLength(25);
      // Nenhuma turma nova aqui — todos os batches são da fase 2 (chunks).
      expect(mockWriteBatch).toHaveBeenCalledTimes(4);
      expect(batchInstances.map(b => b.set.mock.calls.length)).toEqual([8, 8, 8, 1]);
      batchInstances.forEach(b => expect(b.commit).toHaveBeenCalledTimes(1));
    });

    it('cada chunk grava seu próprio audit_log por linha, no batch certo', async () => {
      const { saveSigeReport } = await import('../src/lib/sigeReportService');
      const turmas = Array.from({ length: 10 }, (_, i) => turma({ id: `t${i + 1}`, nome: `Turma ${i + 1}` }));
      setFreshTurmas(turmas);
      const rows = turmas.map(t => rowExisting({ turmaId: t.id, turmaNome: t.nome }));
      const input = baseInput({ rows });
      ...
      expect(mockWriteBatch).toHaveBeenCalledTimes(2); // 8 + 2
      expect(mockQueueAuditLog).toHaveBeenCalledTimes(10);
      const [chunk1, chunk2] = batchInstances;
      expect(mockQueueAuditLog.mock.calls.filter(([batch]) => batch === chunk1)).toHaveLength(8);
      expect(mockQueueAuditLog.mock.calls.filter(([batch]) => batch === chunk2)).toHaveLength(2);
    });

    it('falha no segundo chunk: lança SigeReportPartialSaveError com savedRows do primeiro chunk e pendingRows do restante', async () => {
      const { saveSigeReport, SigeReportPartialSaveError } = await import('../src/lib/sigeReportService');
      const turmas = Array.from({ length: 20 }, (_, i) => turma({ id: `t${i + 1}`, nome: `Turma ${i + 1}` }));
      setFreshTurmas(turmas);
      const rows = turmas.map(t => rowExisting({ turmaId: t.id, turmaNome: t.nome }));
      const input = baseInput({ rows });
      ...
      mockWriteBatch.mockImplementation(() => {
        callCount += 1;
        const set = vi.fn();
        // Chunk 1 (linhas 1-8) comita normalmente; chunk 2 (linhas 9-16) falha.
        const commit = callCount === 1
          ? vi.fn().mockResolvedValue(undefined)
          : vi.fn().mockRejectedValue(new Error('deadline-exceeded'));
        const instance = { set, commit };
      ...
      expect(partialErr.failedChunkIndex).toBe(1); // 0-based — segundo chunk
      expect(partialErr.savedRows).toHaveLength(8); // chunk 1 já commitado
      expect(partialErr.pendingRows).toHaveLength(12); // chunk 2 (8) + chunk 3 (4)
      expect(partialErr.createdTurmas).toHaveLength(0); // nenhuma turma nova neste caso
    });

    it('falha no primeiro chunk quando há turmas novas: createdTurmas continua informado, savedRows vazio', async () => {
      const { saveSigeReport, SigeReportPartialSaveError } = await import('../src/lib/sigeReportService');
      setFreshTurmas([]);
      const rows = [rowNova({ turmaNome: 'Turma Nova Única' })];
      const input = baseInput({ rows });
      ...
      mockWriteBatch.mockImplementation(() => {
        ...
        const set = vi.fn();
        const commit = callCount === 1
          ? vi.fn().mockResolvedValue(undefined) // fase 1 (turma) ok
          : vi.fn().mockRejectedValue(new Error('permission-denied')); // fase 2 (chunk único) falha
        const instance = { set, commit };
        batchInstances.push(instance);
        return instance;
      });
      ...
      expect(partialErr.savedRows).toHaveLength(0);
      expect(partialErr.pendingRows).toHaveLength(1);
      expect(partialErr.failedChunkIndex).toBe(0);
    });

    it('recuperação sem duplicação: retentativa depois de uma falha de chunk usa o estado revalidado e não regrava o que já foi salvo', async () => {
      const { saveSigeReport } = await import('../src/lib/sigeReportService');
      const turmasIniciais = [turma({ id: 't1', nome: 'Turma 1' }), turma({ id: 't2', nome: 'Turma 2' })];
      setFreshTurmas(turmasIniciais);

      // Primeira tentativa: chunk único (2 linhas) falha por completo —
      // nada é gravado (nenhum chunk anterior bem-sucedido).
      mockWriteBatch.mockImplementationOnce(() => {
        const instance = { set: vi.fn(), commit: vi.fn().mockRejectedValue(new Error('unavailable')) };
        batchInstances.push(instance);
        return instance;
      });
      const input = baseInput({
        rows: [rowExisting({ turmaId: 't1', turmaNome: 'Turma 1' }), rowExisting({ turmaId: 't2', turmaNome: 'Turma 2' })],
      });
      await expect(saveSigeReport(input, new Map())).rejects.toThrow();

      // Retentativa: revalida contra o Firestore de novo (mesmas turmas,
      // já existentes de antes — nunca recriadas) e o chunk único agora
      // comita com sucesso.
      batchInstances.length = 0;
      mockWriteBatch.mockReset();
      mockWriteBatch.mockImplementation(() => {
        const instance = { set: vi.fn(), commit: vi.fn().mockResolvedValue(undefined) };
        batchInstances.push(instance);
        return instance;
      });
      setFreshTurmas(turmasIniciais);
      ...
      expect(result.rows).toHaveLength(2);
      expect(result.turmasCreated).toBe(0); // nenhuma turma nova — só turmas já existentes
      expect(mockWriteBatch).toHaveBeenCalledTimes(1); // um único chunk na retentativa
    });
  ...
  // Item 3 do code review do PR #18 (caso de um único chunk — mantido para
  // compatibilidade com o comportamento documentado antes da integração ao
  // PR #19).
  describe('recuperação após falha da fase 2 — caso de um único chunk (SigeReportPartialSaveError)', () => {
    it('fase 1 passa e fase 2 (chunk único) falha: lança SigeReportPartialSaveError com as turmas já criadas', async () => {
      const { saveSigeReport, SigeReportPartialSaveError } = await import('../src/lib/sigeReportService');
      setFreshTurmas([]);
